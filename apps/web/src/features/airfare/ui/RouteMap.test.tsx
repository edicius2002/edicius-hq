 ...
 *
 * `queryWrapper` and not `sharedQueryWrapper`, unlike `useSubdivisions.test`:
 * here the cache is not what is being proved, and one shared between tests
 * would let a country fetched by an earlier test appear in a later one.
 */
...
describe('RouteMap', () => {
  ...
  describe('a route with a stop in it', () => {
  ...
  describe('one arc standing for two watches', () => {
  ...
  /*
   * A place name, an airport and an arc are three things fading on the same
   * rule, so they are read together: opacity comes from the geometry rather
   * than from a `facesViewer` switch that would blink each of them out at 90°.
   */
  describe('fading out towards the limb', () => {
  ...
  describe('the names on the land', () => {
    /**
    ...
    describe('continents giving way to countries', () => {
    ...
    describe('a country broken into its subdivisions', () => {
    ...
    describe('the countries beside the one in the middle', () => {
      /**
    ...
    describe('a name arriving and a name going away', () => {
      ...
      it('brings a country in by fading rather than in the frame its geometry lands on', async () => {
        ...
        const settled = Number(
          (screen.getByText('Loreto') as unknown as SVGTextElement).style.opacity,
        );
  ...
  describe('the overlay keeping step with the land', () => {
  ...
  describe('the colour of a route and the press that opens it', () => {
