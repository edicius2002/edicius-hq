...
import { canCreateAlert, evaluateAlert, sideOf, type TrackedAlert } from './alertCross';
...
describe('canCreateAlert', () => {
  it('refuses a buy whose target the current price already meets', () => {
    expect(canCreateAlert('buy', 250, 240)).toBe(false);
    expect(canCreateAlert('buy', 250, 250)).toBe(false);
  });

  it('allows a buy whose target the current price has not reached yet', () => {
    expect(canCreateAlert('buy', 250, 260)).toBe(true);
  });

  it('refuses a sell whose target the current price already meets', () => {
    expect(canCreateAlert('sell', 250, 260)).toBe(false);
  });

  it('allows a sell whose target the current price has not reached yet', () => {
    expect(canCreateAlert('sell', 250, 240)).toBe(true);
  });

  it('allows anything when there is no quote to judge it against', () => {
    expect(canCreateAlert('buy', 250, undefined)).toBe(true);
    expect(canCreateAlert('sell', 250, undefined)).toBe(true);
  });
});
