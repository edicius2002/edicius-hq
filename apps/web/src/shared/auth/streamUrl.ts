 ...
 * There is no intermediary here: `tailscale serve --https` terminates TLS on
 * the owner's own machine, so the only parties that see the URL are the
 * browser and that PC. **This reasoning does not survive a change of
 * transport.** Routing the API through Cloudflare, ngrok or a Vercel rewrite
 * would put the session token in someone else's logs, and whoever moves it
 * must revisit this.
 *
