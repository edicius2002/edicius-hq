...
`GET /api/sentiment` is covered by the same passkey gate as the other data routes. The API, not the browser, requests CNN's public JSON at `https://production.dataviz.cnn.io/index/fearandgreed/graphdata`, validates the complete aggregate-plus-seven-indicator document and writes the normalized snapshot atomically to `services/api/.local-data/sentiment/snapshot.json` (or the equivalent path below `LOCAL_DATA_DIR`). That file is a disposable market-data cache, not user state or an archive.
...
The integration deliberately sends only `Accept: application/json`. A terminal request from the home environment returned HTTP 418 during implementation, while the public JSON resource and schema were independently observable. Do not add browser-identifying headers, forge `Origin`/`Referer`, replay cookies or scrape rendered HTML to work around that refusal. If CNN changes or withdraws the public endpoint, `/api/sentiment` will return 503 (or the labelled stale snapshot while eligible); that is an operational limitation, not an authentication problem to bypass.
