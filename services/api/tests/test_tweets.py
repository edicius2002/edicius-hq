...
def test_tweets_maps_and_skips_corrupt_rows():
    ...
    (tweets_dir() / "sample.jsonl").write_text(
        "\n".join([json.dumps({"id": "1", "date": "2026-01-01", "text": "anon", "is_reply": True, "like_count": 2}), "bad"])
    )
...
def test_tweets_sort_descending_before_limit():
    tweets_dir().mkdir(parents=True)
    rows = [{"id": "old", "date": "Thu Aug 28 01:54:00 +0000 2026"}, {"id": "new", "date": "Fri Aug 28 05:21:12 +0000 2026"}]
    (tweets_dir() / "ordered.jsonl").write_text("\n".join(json.dumps(row) for row in rows))
    assert [tweet["id"] for tweet in client.get("/api/tweets/ordered?limit=1").json()["tweets"]] == ["new"]
